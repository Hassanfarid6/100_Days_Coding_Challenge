...
function newUesr ():Promise<string> {
...
**Explain & TIP:** `Promise.all()` takes an iterable of Promises and returns a single Promise that resolves
 ...
 rejected if any of the passed Promises are rejected. This method is useful for aggregating the results of 
 multiple promises.*/
...
Promise.all([promise1, promise2, promise3]).then((values) =>console.log(values)) // Outputs: [3, 42, "foo"]
